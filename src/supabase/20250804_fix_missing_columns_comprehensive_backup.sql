...
-- job_board_sources policies (public read access)
CREATE POLICY "Anyone can view active job board sources" ON job_board_sources
    FOR SELECT USING (is_active = true);

-- enhanced_job_searches policies
CREATE POLICY "Users can view their own enhanced job searches" ON enhanced_job_searches
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can create their own enhanced job searches" ON enhanced_job_searches
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own enhanced job searches" ON enhanced_job_searches
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own enhanced job searches" ON enhanced_job_searches
    FOR DELETE USING (auth.uid() = user_id);

-- job_search_results policies
CREATE POLICY "Users can view their own job search results" ON job_search_results
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM enhanced_job_searches 
            WHERE enhanced_job_searches.id = job_search_results.search_id 
            AND enhanced_job_searches.user_id = auth.uid()
        )
    );

CREATE POLICY "System can insert job search results" ON job_search_results
...
CREATE POLICY "Users can update their own job search results" ON job_search_results
    FOR UPDATE USING (
        EXISTS (
            SELECT 1 FROM enhanced_job_searches 
            WHERE enhanced_job_searches.id = job_search_results.search_id 
...
CREATE POLICY "Users can delete their own job search results" ON job_search_results
    FOR DELETE USING (
        EXISTS (
            SELECT 1 FROM enhanced_job_searches 
            WHERE enhanced_job_searches.id = job_search_results.search_id 
...
CREATE POLICY "System can insert search execution logs" ON search_execution_log
    FOR INSERT WITH CHECK (
        EXISTS (
            SELECT 1 FROM enhanced_job_searches 
            WHERE enhanced_job_searches.id = search_execution_log.search_id 
...
CREATE POLICY "System can update search execution logs" ON search_execution_log
    FOR UPDATE USING (
        EXISTS (
            SELECT 1 FROM enhanced_job_searches 
            WHERE enhanced_job_searches.id = search_execution_log.search_id 
...
END
$$;
-- Add unique constraint on name column (required for ON CONFLICT)
ALTER TABLE job_board_sources 
